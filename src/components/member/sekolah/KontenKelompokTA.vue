      ...
      <div class="ui relaxed divided list">
        <div class="item" v-for="universitas in universitas_mahasiswa">
          <div class="content">
            ...
            <div class="description">Jenjang: {{universitas.jenjang.nama}} | Prodi: {{universitas.prodi.nama}} | NIM: {{universitas.nim_nip}}</div>            
            <a class="ui yellow label" v-if="universitas.status_konfirmasi == 0">Mohon tunggu dalam proses konfirmasi oleh Universitas/ Institusi pendidikan Anda</a>
            <a class="ui green label" v-else-if="universitas.status_konfirmasi == 1" href="#">              
              <span v-if="universitas.kelompok == null">                
               <router-link :to="{name: 'mahasiswa_gabung_kelompok',params:{universitas_prodi_mahasiswa_id:universitas._id,universitas_prodi_mahasiswa_universitas:universitas.universitas._id}}" style="color:white;"> Gabung kelompok Tugas Akhir disini</router-link>
              </span>
              <span v-else-if="universitas.kelompok != null">
                <!-- <router-link to="/kelompok_ta/detail" class="item grey-text">
                 -->
                 <router-link :to="{name: 'kelompok_ta_detail',params:{kelompok:universitas.kelompok}}" class="item grey-text">
                  Menuju menu kelompok TA
                </router-link>
              </span>              
            </a>
          </div>
    ...
    created() {
      ...
      this.$http.post(global_json.general_url + '/mahasiswa/daftar_universitas', {
        ...
        if (data.body.success == true) {
          ...
          //console.log("OOOOO: "+JSON.stringify(data.body.data.data[0].akademik.universitas_prodi.length))
